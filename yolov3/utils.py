...
def save_cropped_image(cropped_image, image_name, index):
    ...
    path = os.path.join(os.getcwd(), "IMAGES", image_name + "_dir")
...
def draw_bbox(image, bboxes, CLASSES=YOLO_COCO_CLASSES, show_label=True, show_confidence = True, Text_colors=(255,255,0), rectangle_colors='', tracking=False, image_name=None):
    ...
    for i, bbox in enumerate(bboxes):
        ...
        save_cropped_image(crop_img, image_name, index)
...
def detect_image(Yolo, image_path, output_path, input_size=416, show=False, CLASSES=YOLO_COCO_CLASSES, score_threshold=0.3, iou_threshold=0.45, rectangle_colors=''):
    ...
    image_name_parts = image_parts[-1].split(".")
    image = draw_bbox(original_image, bboxes, CLASSES=CLASSES, rectangle_colors=rectangle_colors, image_name=image_name_parts[0])
